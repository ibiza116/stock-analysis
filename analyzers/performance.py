...
class PerformanceAnalyzer:
    ...
    def _calculate_market_comparison(self, trades, price_data, initial_capital):
        ...
        market_variance = np.var(price_data['Close'].pct_change().dropna())
        strategy_market_covariance = np.cov(strategy_returns, price_data['Close'].pct_change().dropna()[:len(strategy_returns)])[0][1]
        beta = strategy_market_covariance / market_variance if market_variance > 0 else 1
